...
filo_length_details = dicty_filo %>%
  ...
  summarize(Filo_Length_Avg = mean(Filo_Length, na.rm =  T),
            ...
            Tip_Avg = mean(Tip_Intensity_Sum, na.rm = T),
            Tip_SD = sd(Tip_Intensity_Sum, na.rm = T)
            )

mytable_actin = cbind(totalcell_details[1:7],
                      filopercell_details[3:4],
                      filo_length_details[3:7]) %>%
...
(ax2_cortex= dicty_total %>%
   ...
   geom_errorbar(data =ax2,
                 aes(y = CortBody_Ratio_Avg,
                     group_by = Plasmid,
                     ymin = CortBody_Ratio_Avg - CortBody_Ratio_SD/sqrt(Total_Cells),
                     ymax = CortBody_Ratio_Avg + CortBody_Ratio_SD/sqrt(Total_Cells)),
                 position = position_dodge(width = 0.9),
                 color = "black",
                 alpha = 0.5,
                 size = 1,
...
(ax2_length = ax2_filo %>%
   ...
   geom_errorbar(data = ax2,
                 aes(y = Filo_Length_Avg,
                     group_by = Plasmid,
                     ymin = Filo_Length_Avg - Filo_Length_SD/sqrt(Total_Cells),
                     ymax = Filo_Length_Avg + Filo_Length_SD/sqrt(Total_Cells)),
                 position = position_dodge(width = 0.9),
                 color = "black",
                 alpha = 0.5,
                 size = 1,
...
(ax2_tipint = ax2_filo %>%
   ...
   geom_errorbar(data = ax2,
                 aes(y = Tip_Avg,
                     group_by = Plasmid,
                     ymin = Tip_Avg - Tip_SD/sqrt(Total_Cells),
                     ymax = Tip_Avg + Tip_SD/sqrt(Total_Cells)),
                 position = position_dodge(width = 0.9),
                 color = "black",
                 alpha = 0.5,
                 size = 1,
    ...
    scale_color_manual(breaks = c("pDTi516", "pDTi571", "pDTi500"),
                         values = c("#00ba39", "#609cff", "#f8766d")) +
    labs(y = "Filopodia Tip Intensity (A.U.)",
         x = "Cell Line")
)
```
